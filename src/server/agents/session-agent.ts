...
export class SessionAgent extends Agent<Env, SessionAgentState> {
   ...
   *  model/mode-switch case, where session.get confirms the id — stay silent because context is intact. */
  ...
  private async ensureOpencode(): Promise<void> {
    ...
    // preset only apply at process start). Session history lives in OpenCode's SQLite db (pinned
    // into /workspace below), so the same sessionID resumes with full context after the restart —
    // verified against the real 0.12.7-opencode image (opencode 1.18.18). (ocModelDirty is the
    // flag's pre-rename key; honor a pending one across the deploy.)
    // The bracketed kill pattern can never match this exec's own wrapper (a bare `pkill -f
    // opencode` SIGTERMs its own `sh -lc`, whose cmdline contains "opencode"), and the pgrep loop
    // replaces a blind 500ms sleep: createOpencode reuses any process the registry still reports
    // as running, so racing a dying server would silently keep the OLD config.
